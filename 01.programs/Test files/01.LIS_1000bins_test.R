...
#-- Same surveys as LIS_1000bins_test.do (testing with SampleData) ----------
path    <- "C:/WBG/Git repos/Packages/GPID/PIP/LIS_data/01.programs/SampleData"
surveys <- c("us14ih", "us16ih", "mx14ih", "mx16ih", "it14ih", "it16ih")
files   <- file.path(path, paste0(surveys, ".dta"))

## LIS function (reads local .dta files instead of LISSY remote)
...
LIS_bins_data <- function(file){
  ...
  tryCatch(
    expr = {
      ...
      # Currency: decode label then take second "-" segment, fallback to first
      curr_label   <- as.character(haven::as_factor(funique(data_clean$currency)))
      curr_parts   <- tstrsplit(curr_label, "-", fixed = TRUE)
      currency_val <- if (length(curr_parts) >= 2 && !is.na(curr_parts[[2]]) && curr_parts[[2]] != "") curr_parts[[2]] else curr_parts[[1]]
      ...
      final_data <- bin_data |>
        fmutate(country_code  = toupper(funique(data_clean$iso3)),
                ...
                min_welfare   = fmin(data_clean$welfare),
                max_welfare   = fmax(data_clean$welfare),
...
# Run for all files
final <- rbindlist(lapply(files, LIS_bins_data), use.names = TRUE)
nrow(final) # should be length(surveys) * 1000

# Save as CSV for comparison with Stata output
today <- format(Sys.Date(), "%d_%b_%Y")
out_file <- file.path(path, paste0("wb_1000bin_append_", today, ".csv"))
